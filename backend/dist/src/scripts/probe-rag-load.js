...
async function probe(round, key, model) {
    const llm = new google_genai_1.ChatGoogleGenerativeAI({ model, apiKey: key, temperature: 0.3 });
    const t0 = Date.now();
    try {
        const res = await withTimeout(llm.invoke([{ role: 'user', content: BIG_PROMPT }]), TIMEOUT_MS, `chat-rag-${round}`);
        const ms = Date.now() - t0;
        const text = typeof res.content === 'string' ? res.content : JSON.stringify(res.content);
        return { ok: true, ms, sample: text.slice(0, 80) };
    }
    catch (err) {
