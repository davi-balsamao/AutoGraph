...
exports.baseHandlerUtils = void 0;
exports.isOffTopicMessage = isOffTopicMessage;
exports.prepareContext = prepareContext;
exports.detectCrossCuttingIntent = detectCrossCuttingIntent;
exports.enrichForLeigo = enrichForLeigo;
exports.isAudioMessage = isAudioMessage;
exports.validateDiscountMargin = validateDiscountMargin;
exports.getMaxDiscountPercent = getMaxDiscountPercent;
 ...
 */
const OFF_TOPIC_RE = /\b(piada|piadas|receita|receitas|bolo|bolos|chocolate|futebol|m[úu]sica|m[úu]sicas|filme|filmes|s[ée]rie|s[ée]ries|hobby|hobbies|namorad[oa]s?|fofoca|cl[ií]ma|previs[ãa]o do tempo|signo|hor[óo]scopo)\b/i;
...
const ESCALAR_RE = /\b(atendente|humano|gerente|pessoa|falar com algu[ée]m)\b/i;
const NOVO_RE = /\b(novo atendimento|come[çc]ar de novo|reiniciar|outro pedido|esquece tudo|cancela tudo)\b/i;
// Pedido explícito de pausa — Regra 6 estendida (Fluxo 6).
const PAUSE_RE = /\b(preciso parar|vou parar|pausa|pausar|continue (meu pedido )?depois|retorno depois|volto (mais )?(tarde|depois)|n[ãa]o posso (agora|continuar)|ocupad[oa] agora|tenho que sair)\b/i;
...
const LISTAR_PEDIDOS_RE = /\b(meus pedidos|minhas (ordens|os|encomendas|compras)|que (pedidos|ordens|encomendas) eu (j[áa] )?fiz|quais (pedidos|ordens|encomendas) eu (j[áa] )?fiz|hist[óo]rico (de|dos) (pedidos|ordens|compras)|pedidos (anteriores|antigos|que eu (j[áa] )?fiz)|meus or[çc]amentos)\b/i;
 ...
 */
async function prepareContext(_message, sessao, deps) {
    const history = deps.conversationHistory || `Cliente: ${_message}`;
    const entities = await entity_extraction_service_1.entityExtractionService.extract(history, {
        produtoAtual: sessao.contexto.produto,
    });
    const context = (0, context_util_1.syncContextFromEntities)({ ...sessao.contexto }, entities);
 ...
 */
function detectCrossCuttingIntent(message) {
    if (!message)
        return null;
    if (NOVO_RE.test(message))
        return 'NOVO';
    if (ESCALAR_RE.test(message))
        return 'ESCALAR';
    ...
    if (LISTAR_PEDIDOS_RE.test(message))
        return 'LISTAR_PEDIDOS';
    if (transition_service_1.DUVIDA.test(message))
        return 'DUVIDA';
    return null;
}
/**
 ...
 */
function enrichForLeigo(resposta, contexto) {
    if (!resposta)
        return resposta;
 ...
 */
function isAudioMessage(messageData) {
 ...
 */
function validateDiscountMargin(propostaPercentual, margemMaxima) {
    if (!Number.isFinite(propostaPercentual) || !Number.isFinite(margemMaxima)) {
        return false;
    }
    if (propostaPercentual < 0)
        return false;
 ...
 */
function getMaxDiscountPercent(orderTotal) {
