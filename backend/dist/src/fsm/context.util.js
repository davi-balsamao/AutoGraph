...
function isRespostaValida(valor) {
...
function montarPendentesPreservandoContexto(perguntasFaltantes, specs) {
...
function syncContextFromEntities(context, entities) {
    ...
    if (entities.requisitos.length > 0) {
         ...
         */
        updated.specsPendentes = montarPendentesPreservandoContexto(entities.perguntasFaltantes, specs);
        const arteReq = entities.requisitos.find((r) => normalizarTexto(r.pergunta).includes('arte pronta'));
