...
exports.guardrailsService = exports.GuardrailsService = void 0;
const PRICE_TOLERANCE = 0.02;
const TRANSITION_NO_PRICE = 'Já anotei tudo! Vou processar seu orçamento e enviar para a nossa equipe aprovar no sistema. Assim que liberado, te passo o valor aqui mesmo, ok?';
const HALLUCINATION_PRICE_MESSAGE = 'Para te passar o valor correto, preciso confirmar todos os detalhes do pedido com base na nossa tabela. Pode me ajudar com as especificações que ainda faltam?';
class GuardrailsService {
    ...
    outOfScopeMessage = 'Desculpe, como assistente da AutoGraph, só posso ajudar com informações sobre nossos produtos gráficos, preços, prazos e materiais de impressão. Como posso ajudar com seu projeto gráfico?';
    /**
     ...
     */
    validateResponse(response, retrievedDocs, state) {
        ...
        const invalidPrices = generatedPrices.filter((price) => !this.isPriceAuthorized(price, authorizedPrices));
        if (invalidPrices.length > 0) {
            console.warn(`🚨 [Guardrails] Preço não autorizado (ex: R$ ${invalidPrices[0]}). Valores da KB: ${authorizedPrices.slice(0, 5).join(', ')}...`);
            return {
                isValid: false,
    ...
    extractAuthorizedPrices(retrievedDocs) {
     ...
     */
    isPriceAuthorized(price, authorizedPrices) {
    ...
    getMaxDiscountPercent(orderTotal) {
        if (orderTotal < 200)
            return 0;
        if (orderTotal < 500)
            return 5;
...
exports.guardrailsService = new GuardrailsService();
