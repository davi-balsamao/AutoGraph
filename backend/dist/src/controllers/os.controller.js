...
class OsController {
    async create(req, res) {
        try {
            ...
            let specsObj = typeof especificacoes === 'string' ? JSON.parse(especificacoes) : especificacoes;
            if (req.file)
                specsObj.arteUrl = `/uploads/${req.file.filename}`;
            const novaOs = await osRepo.create({ clienteId, status: client_1.StatusOS.CRIADA, especificacoes: specsObj, observacoes: observacoes || null });
    ...
    async updateStatus(req, res) {
        try {
            ...
            server_1.io.emit('kanban_atualizado', osAtualizada);
    ...
    async enviarMensagemGerente(req, res) {
        try {
            ...
            server_1.io.emit(`chat-${cliente.id}`, { id: mensagem.id, origem: 'GERENTE', texto: texto, criadoEm: mensagem.criadoEm });
    ...
    async startTimer(req, res) {
        try {
            ...
            server_1.io.emit('kanban_atualizado', os);
            return res.json(os);
        }
        catch (error) {
