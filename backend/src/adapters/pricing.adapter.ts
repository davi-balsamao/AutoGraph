...
function aliasesDoProduto(produtoNome: string): string[] {
...
function textoDasSpecs(specs?: any): string {
  if (!specs) return '';
...
function extrairDimensoesEmMetros(specs?: any): { larguraM: number; alturaM: number } | null {
  const textoOriginal = textoDasSpecs(specs);
  const texto = normalizar(textoOriginal);

  const medida = texto.match(
    /(\d+(?:[.,]\d+)?)\s*x\s*(\d+(?:[.,]\d+)?)(?:\s*(cm|m|metro|metros))?/i
  );
...
function calcularPrecoBanner(precoBase: number, quantidade: number, specs?: any): number {
  ...
  static async calcular(input: OrcamentoInput): Promise<number> {
    ...
    const produtoDb = produtos.find((produto) => {
      ...
      return aliases.some((alias) => {
        ...
        return (
          nomeDb.includes(aliasNormalizado) ||
    ...
    if (!produtoDb) {
      ...
      throw new Error(
        `Produto não encontrado no catálogo. Produto recebido: "${produtoNome}". Produtos disponíveis: ${disponiveis}`
      );
    }

    let precoFinal = calcularPrecoBase(
      produtoDb.nome,
      produtoDb.precoBase,
      quantidade,
      input.specs
     ...
     */
    if (
      acabamentoNormalizado.includes('fosco') ||
