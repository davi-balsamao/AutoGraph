...
describe('ConfirmarPedidoHandler', () => {
  ...
  const ctxBase = {
    ...
    entrega: { modalidade: 'retirada' as const },
  };

  it('emite resumo determinístico quando mensagem chega vazia (chain entry)', async () => {
    const sessao = makeSessao({ contexto: ctxBase });
    const r = await confirmarPedidoHandler.handle('', sessao, makeDeps());

    expect(r.nextState).toBe(ConversationState.CONFIRMAR_PEDIDO);
  ...
  it('encadeia para GERAR_OS quando cliente confirma', async () => {
    const sessao = makeSessao({ contexto: ctxBase });
    const r = await confirmarPedidoHandler.handle('Confirmo o pedido.', sessao, makeDeps());

    expect(r.nextState).toBe(ConversationState.GERAR_OS);
    expect(r.chainNext).toBe(ConversationState.GERAR_OS);
    expect(r.response).toBe('');
  });

  it('volta a COLETAR_ESPECIFICACOES quando cliente quer corrigir', async () => {
    const sessao = makeSessao({ contexto: ctxBase });
    const r = await confirmarPedidoHandler.handle(
      'tá errado, quero trocar a quantidade',
      sessao,
      makeDeps()
    );
  ...
  it('re-pergunta com o resumo quando resposta é ambígua', async () => {
    const sessao = makeSessao({ contexto: ctxBase });
    const r = await confirmarPedidoHandler.handle('hmmm', sessao, makeDeps());
