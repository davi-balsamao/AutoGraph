...
describe('GerarOsHandler', () => {
  ...
  it('cria O.S. com produto e specs do contexto', async () => {
    ...
    const r = await gerarOsHandler.handle('', sessao, makeDeps({ clienteNome: 'Davi' }));
  ...
  it('Fase 2: NÃO grava observações técnicas (recepcionista preenche)', async () => {
    ...
    await gerarOsHandler.handle('', sessao, makeDeps({ clienteNome: 'Ana' }));
  ...
  it('retorna nextState=ENCERRAR e flag gerarOs=true', async () => {
    const sessao = makeSessao({ contexto: { produto: 'Panfletos' } });
    const r = await gerarOsHandler.handle('', sessao, makeDeps());
