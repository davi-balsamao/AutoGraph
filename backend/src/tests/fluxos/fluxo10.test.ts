...
describe('Fluxo 10 · Produto fora do catálogo', () => {
  ...
  it('deve informar produto indisponível e encerrar com educação', async () => {
    await turno(
      PHONE,
      NAME,
      'Oi! Quero copos personalizados com o logo da minha empresa.',
      'IDENTIFICAR_NECESSIDADE',
      'Boas-vindas + produto indisp.'
    );

    await turno(
      PHONE,
      NAME,
      'Preciso de 100 copos com meu logo impresso.',
      'PRODUTO_INDISPONIVEL',
      'Identificar → Prod. indisp.'
    );

    await turno(
      PHONE,
      NAME,
      'Entendido, vocês não fazem copos. Tem alguma alternativa impressa?',
      'PRODUTO_INDISPONIVEL',
      'Alternativa'
    );
    ...
    await turno(
      PHONE,
      NAME,
      'Tudo bem, por enquanto não preciso de mais nada. Obrigado!',
      'ENCERRAR',
      'Encerrar'
    );
  }, 120_000);
