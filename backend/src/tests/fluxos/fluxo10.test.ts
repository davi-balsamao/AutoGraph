/**
 * Teste E2E: Fluxo 10 · Produto completamente fora do catálogo
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo10.md
 */
...
describe('Fluxo: fluxo10', () => {
  ...
  it('deve processar O FLUXO COMPLETO (4 passos)', async () => {
    ...
    // Passo 3: Prod. indisp.
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Prod. indisp.');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
  ...
  }, 70000); // Timeout dinâmico baseado no número de passos
