/**
 * Teste E2E: Fluxo 11 · Múltiplas dúvidas antes de qualquer spec
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo11.md
 */
...
describe('Fluxo: fluxo11', () => {
  ...
  it('deve processar O FLUXO COMPLETO (14 passos)', async () => {
    ...
    // Passo 3: Esclarecer
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Esclarecer');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
    ...
    // Passo 7: Validar arq.
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Validar arq.');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
    ...
    const botResponse12 = await getLastBotResponse(testNumber);
    expect(botResponse12).toBeTruthy();
    console.log('[Turno 12] IA respondeu:', botResponse12);
    
    // Passo 13: Gerar O.S.
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Gerar O.S.');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA

    const botResponse13 = await getLastBotResponse(testNumber);
    expect(botResponse13).toBeTruthy();
    console.log('[Turno 13] IA respondeu:', botResponse13);
    
    // Passo 14: Encerrar
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Encerrar');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA

    const botResponse14 = await getLastBotResponse(testNumber);
    expect(botResponse14).toBeTruthy();
    console.log('[Turno 14] IA respondeu:', botResponse14);
    
  }, 220000); // Timeout dinâmico baseado no número de passos
