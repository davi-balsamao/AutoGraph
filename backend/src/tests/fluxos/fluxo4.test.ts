...
describe('Fluxo 4 · Urgência — entrega em 24h com negociação', () => {
  ...
  it('deve esclarecer custo, validar arquivo, negociar e fechar o pedido', async () => {
    ...
    await turno(PHONE, NAME, 'Qual o custo para entrega expressa em 24 horas?', 'ESCLARECER_DUVIDA', 'Pergunta custo cedo → Esclarecer', 30_000);
    ...
    await turno(PHONE, NAME, 'Entendi. Sim, tenho o arquivo de arte pronto em PDF.', 'AGUARDAR_APROVACAO', 'Confirma arte → chain Calcular', 30_000);
    ...
    await turno(PHONE, NAME, 'Confirmo o pedido urgente.', 'ENCERRAR', 'Confirmar → Gerar O.S.', 30_000);
