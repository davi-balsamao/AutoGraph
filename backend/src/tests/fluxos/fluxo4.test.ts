/**
 * Teste E2E: Fluxo 4 · Urgência que altera custo e prazo
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo4.md
 */
...
describe('Fluxo: fluxo4', () => {
  ...
  it('deve processar O FLUXO COMPLETO (12 passos)', async () => {
    ...
    // Passo 4: Calcular expresso
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Calcular expresso');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
    ...
    // Passo 7: Negociar
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Negociar');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
