/**
 * Teste E2E: Fluxo 17 · Canal não suportado pelo bot
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo17.md
 */
...
describe('Fluxo: fluxo17', () => {
  ...
  it('deve processar O FLUXO COMPLETO (12 passos)', async () => {

    // Passo 1: Boas-vindas áudio ❌
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Boas-vindas áudio ❌');
    ...
    // Passo 2: Boas-vindas solicita texto
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Boas-vindas solicita texto');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
  ...
  }, 190000); // Timeout dinâmico baseado no número de passos
