...
describe('Fluxo 14 · Ambiguidade no produto solicitado', () => {
  ...
  it('deve pedir clareza sobre o produto e seguir para aprovação admin', async () => {
    await turno(
      PHONE,
      NAME,
      'Oi, preciso de cartão.',
      'IDENTIFICAR_NECESSIDADE',
      'Boas-vindas + ambiguidade'
    );

    await turno(
      PHONE,
      NAME,
      'Cartão de visita para a minha empresa.',
      'COLETAR_ESPECIFICACOES',
      'Identificar clarificado'
    );

    await turno(
      PHONE,
      NAME,
      '500 unidades, formato 9x5cm, só frente colorida e verso em branco, papel couchê 300g.',
      'VALIDAR_ARQUIVO',
      'Coletar specs básicas'
    );

    await sendMsg(PHONE, NAME, 'Tenho o arquivo em PDF com sangria de 3mm.');
    await wait(30_000);
