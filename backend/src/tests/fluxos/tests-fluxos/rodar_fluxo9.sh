...
# Script para automatizar o teste do Fluxo 9
# Ele inicia o banco, sobe o backend em segundo plano, executa o teste Jest,
# salva os resultados em 'resultado_fluxo9.txt' e depois encerra o backend.
...
if [ "$server_ready" = true ]; then
  ...
  echo -e "${YELLOW}🧪 [3/4] Executando o teste do Fluxo 9...${NC}"
  # Executa o jest direcionando a saída tanto para o terminal quanto para o arquivo resultado_fluxo9.txt na raiz
  npx jest src/tests/fluxos/fluxo9.test.ts --forceExit 2>&1 | tee ../resultado_fluxo9.txt
  
  echo -e "\n${GREEN}💾 Resultado do teste salvo em: ${YELLOW}resultado_fluxo9.txt${NC}"
else
