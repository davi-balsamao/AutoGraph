...
# Script para automatizar o teste do Fluxo 11
# Ele inicia o banco, sobe o backend em segundo plano, executa o teste Jest,
# salva os resultados em 'resultado_fluxo11.txt' e depois encerra o backend.
...
if [ "$server_ready" = true ]; then
  ...
  echo -e "${YELLOW}🧪 [3/4] Executando o teste do Fluxo 11...${NC}"
  # Executa o jest direcionando a saída tanto para o terminal quanto para o arquivo resultado_fluxo11.txt na raiz
  npx jest src/tests/fluxos/fluxo11.test.ts --forceExit 2>&1 | tee ../resultado_fluxo11.txt
  
  echo -e "\n${GREEN}💾 Resultado do teste salvo em: ${YELLOW}resultado_fluxo11.txt${NC}"
else
