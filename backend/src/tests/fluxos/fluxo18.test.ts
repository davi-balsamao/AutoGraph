/**
 * Teste E2E: Fluxo 18 · Idioma diferente do configurado
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo18.md
 */
...
describe('Fluxo: fluxo18', () => {
  ...
  it('deve processar O FLUXO COMPLETO (10 passos)', async () => {

    // Passo 1: Boas-vindas es
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Boas-vindas es');
    ...
    // Passo 2: Identificar bilíngue
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: Identificar bilíngue');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
  ...
  }, 160000); // Timeout dinâmico baseado no número de passos
