/**
 * Teste E2E: Fluxo 20 · Uso fora do propósito do bot
 * Baseado no fluxo: backend/rag-test/fluxos/fluxo20.md
 */
...
describe('Fluxo: fluxo20', () => {
  ...
  it('deve processar O FLUXO COMPLETO (3 passos)', async () => {

    // Passo 1: (qualquer)
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: (qualquer)');
    ...
    // Passo 2: (redirect)
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: (redirect)');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
    ...
    // Passo 3: (mesmo estado)
    await sendMessage(testNumber, testName, 'Simulando cliente avançando no passo: (mesmo estado)');
    await new Promise(r => setTimeout(r, 8000)); // Espera processamento IA
  ...
  }, 55000); // Timeout dinâmico baseado no número de passos
