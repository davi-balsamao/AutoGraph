...
import { parseLlmResponse } from '../../services/llm-entity-extractor.service';
...
describe('parseLlmResponse', () => {
  ...
  it('normaliza intent inválido para OUTRO', () => {
    ...
    expect(parseLlmResponse(raw).intent).toBe('OUTRO');
  });
  ...
  it('lança erro em JSON inválido', () => {
    expect(() => parseLlmResponse('isso nem é JSON')).toThrow(/não-JSON/);
  });

  it('lança erro quando saída é JSON mas não-objeto', () => {
    expect(() => parseLlmResponse('"apenas uma string"')).toThrow();
  });
});
