...
async function runTests() {
  ...
  const result1 = guardrailsService.validateResponse(response1, mockDocs);
  ...
  const result2 = guardrailsService.validateResponse(response2, mockDocs);
  ...
  const result3 = guardrailsService.validateResponse(response3, mockDocs);
  ...
  const result4 = guardrailsService.validateResponse(response4, mockDocs);
  ...
  const result5 = guardrailsService.validateResponse(response5, mockDocs);
  ...
  const result6 = guardrailsService.validateResponse(response6, mockDocs);
  ...
  const result7 = guardrailsService.validateResponse(response7, mockDocs);
  ...
  const response8 =
    'Consigo fechar em R$ 266,00 no Pix para você agora. O panfleto couché 115g fica nesse valor com a parceria.';
  const mockDocsPanfleto: Document[] = [
    new Document({
      pageContent: 'Panfleto Couché 115g 14x20cm 4x4 — 1000 un: R$ 280,00.',
      metadata: { source: 'tabela_precos_grafica.md' },
    }),
  ];
  const result8 = guardrailsService.validateResponse(response8, mockDocsPanfleto);
  ...
  const result9 = guardrailsService.validateResponse(response9, mockDocsPanfleto);
