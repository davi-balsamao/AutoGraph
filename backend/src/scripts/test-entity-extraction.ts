...
console.log(`📦 Produtos disponíveis: ${entityExtractionService.getAvailableProducts().join(', ')}\n`);
...
for (const tc of testCases) {
  ...
  console.log(
    `  Produto: ${resultado.produtoIdentificado || '(nenhum)'} ${produtoCorreto ? '✅' : `❌ (esperado: ${tc.produtoEsperado})`}`
  );
