...
function respostaCurta(message: string): boolean {
...
function palavraNumeroParaDigito(message: string): string {
...
function formatarRespostaParaPergunta(pergunta: string, message: string): string {
  ...
  if (perguntaNorm.includes('quantidade')) {
    if (/^\d{1,6}$/.test(numeroConvertido)) return `${numeroConvertido} unidades`;
    return msgOriginal;
  }

  if (perguntaNorm.includes('vias')) {
    if (/^\d{1,2}$/.test(numeroConvertido)) return `${numeroConvertido} vias`;
    return msgOriginal;
  }
  ...
  if (perguntaNorm.includes('autocopiativo')) {
    if (/^(sim|s)$/i.test(msgNorm)) return 'Sim, autocopiativo';
    if (/^(nao|não|n)$/i.test(msgNorm)) return 'Não';
    return msgOriginal;
  }

  if (perguntaNorm.includes('numerado')) {
    if (/^(sim|s)$/i.test(msgNorm)) return 'Sim, numerado';
    if (/^(nao|não|n)$/i.test(msgNorm)) return 'Não';
    return msgOriginal;
  }

  if (perguntaNorm.includes('cor') || perguntaNorm.includes('colorido') || perguntaNorm.includes('preto')) {
    if (msgNorm.includes('colorid')) return 'Colorido';
    if (msgNorm.includes('preto') || msgNorm === 'pb' || msgNorm === 'p/b') return 'Preto e branco';
    return msgOriginal;
  }

  if (perguntaNorm.includes('verniz')) {
    if (/^(sim|s)$/i.test(msgNorm)) return 'Sim, com verniz total';
    if (/^(nao|não|n)$/i.test(msgNorm)) return 'Sem verniz total';
    return msgOriginal;
  }

  if (perguntaNorm.includes('lamina')) {
    if (/^(sim|s)$/i.test(msgNorm)) return 'Com laminação fosca';
    if (/^(nao|não|n)$/i.test(msgNorm)) return 'Sem laminação';
    return msgOriginal;
  }
...
function aplicarRespostaCurtaNaPerguntaAtual(
...
function extrairQuantidadeDasSpecs(specs: any): number {
  if (!specs) return 0;

  const quantidadeEntry = Object.entries(specs).find(([pergunta]) =>
    normalizarTexto(pergunta).includes('quantidade')
  );
...
function quantidadeMinimaProduto(produto?: string | null): number {
  const p = normalizarTexto(produto);

  if (p.includes('panfleto') || p.includes('flyer')) return 100;
  if (p.includes('cartao')) return 100;

  return 0;
}

function validarQuantidadeMinima(context: ConversationContext): string | null {
  ...
  if (!quantidade || quantidade >= minimo) return null;

  return `Para ${context.produto}, a quantidade mínima é de ${minimo} unidades. Posso ajustar seu pedido para ${minimo} unidades?`;
}

function respostaAceitaAjusteMinimo(message: string): boolean {
  return /\b(sim|pode|pode sim|ok|certo|beleza|perfeito|ajusta|ajustar|fechado|combinado|claro|manda|vamos)\b/i.test(
    message
  );
}

function aplicarAjusteQuantidadeMinimaSeAceito(
  context: ConversationContext,
  message: string,
  perguntaAtual?: string | null
): ConversationContext {
  if (!respostaAceitaAjusteMinimo(message)) return context;

  const minimo = quantidadeMinimaProduto(context.produto);

  if (!minimo) return context;

  const quantidadeAtual = extrairQuantidadeDasSpecs(context.specs);

  const deveAjustar =
    perguntaAtual === AJUSTE_QUANTIDADE_MINIMA ||
    (quantidadeAtual > 0 && quantidadeAtual < minimo);

  if (!deveAjustar) return context;

  const specs = { ...(context.specs || {}) };

  const chaveQuantidade =
    Object.keys(specs).find((k) => normalizarTexto(k).includes('quantidade')) ||
  ...
  return {
    ...context,
    specs,
    specsPendentes: (context.specsPendentes || []).filter(
      (p) =>
        p !== AJUSTE_QUANTIDADE_MINIMA &&
        !normalizarTexto(p).includes('quantidade')
    ),
  };
}

function aplicarProdutoTravado(
  context: ConversationContext,
  produtoTravado?: string | null
): ConversationContext {
  if (!produtoTravado) return context;
...
function reforcarQuantidadeDaMensagem(
  context: ConversationContext,
  message: string
): ConversationContext {
  ...
  const match = msg.match(
    /\b(\d{1,6})\s*(?:unidades?|unid|und|panfletos?|cartões?|cartoes?|blocos?|banners?)?\b/i
  );
  ...
  const chaveQuantidade = Object.keys(specs).find((k) =>
    normalizarTexto(k).includes('quantidade')
  );
  ...
  const specsPendentes = (context.specsPendentes || []).filter(
    (p) => !normalizarTexto(p).includes('quantidade')
  );
...
function removerPendenciasJaPreenchidas(context: ConversationContext): ConversationContext {
  const specs = context.specs || {};

  const specsPendentes = (context.specsPendentes || []).filter((pergunta) => {
    if (pergunta === AJUSTE_QUANTIDADE_MINIMA) return true;
...
function perguntaHumanizada(pergunta: string): string {
  ...
  if (p.includes('quantidade')) return 'Perfeito. Quantas unidades você precisa?';
  if (p.includes('tamanho') || p.includes('formato')) return 'Certo. Qual tamanho/formato você quer? Pode ser A4, A5 ou uma medida em cm.';
  if (p.includes('vias')) return 'Entendi. Cada folha terá quantas vias?';
  if (p.includes('autocopiativo')) return 'O papel será autocopiativo?';
  if (p.includes('cor') || p.includes('colorido') || p.includes('preto')) return 'A impressão será colorida ou preto e branco?';
  if (p.includes('numerado')) return 'O bloco será numerado?';
  if (p.includes('verniz')) return 'Terá verniz total?';
  if (p.includes('lamina')) return 'Terá laminação?';

  return formatarPerguntaCatalogo(pergunta);
}

function ehPerguntaDeDuvida(message: string): boolean {
  return /\?|(\b(o que é|o que e|como funciona|pra que serve|para que serve|qual a diferença|qual a diferenca|não entendi|nao entendi|explica|me explica|isso o que é|isso o que e|o que é isso|o que e isso)\b)/i.test(
    message
  );
}

function respostaRapidaDuvidaGrafica(
  message: string,
  perguntaAtual?: string | null
): string | null {
...
export class ColetarEspecificacoesHandler implements StateHandler {
  async handle(
    ...
    if (nextState === ConversationState.ESCLARECER_DUVIDA || ehPerguntaDeDuvida(message)) {
      ...
      try {
        ...
        return {
          response:
            'Posso te explicar sim. Essa é uma informação técnica do pedido. Me diga se você quer seguir com essa opção ou prefere que eu continue com uma alternativa mais simples.',
          nextState: ConversationState.COLETAR_ESPECIFICACOES,
          updatedContext: context,
        };
      }
      ...
      return {
        ...
        updatedContext: {
          ...
          specsPendentes: [
            ...
            ...(context.specsPendentes || []).filter(
              (p) => p !== AJUSTE_QUANTIDADE_MINIMA
            ),
          ],
        },
      };
